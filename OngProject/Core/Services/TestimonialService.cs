                    ...
                    if (testimonialDto.Image != null)
                    {
                        if (ValidateFiles.ValidateImage(testimonialDto.Image))
                        {
                            ...
                            if (testimonial.Image != null)
                                result = await s3helper.AwsUploadFile(testimonial.Image.Substring(testimonial.Image.IndexOf("testimonial_")), testimonialDto.Image);
                            else
                            {
                                string key = DateTime.Now.ToString();
                                key = key.Replace(":", "").Replace("/", "").Replace(" ", "");
                                result = await s3helper.AwsUploadFile("testimonial_" + key, testimonialDto.Image);
                            }
                            if (result.Code == 200)
                            {
